/*
 * Teamwork.com Projects API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * API version: 3.0
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package projv3

import (
	"encoding/json"
)

// CalendarEventsResponseIncluded struct for CalendarEventsResponseIncluded
type CalendarEventsResponseIncluded struct {
	Projects *map[string]ViewProject `json:"projects,omitempty"`
	Users *map[string]ViewUser `json:"users,omitempty"`
}

// NewCalendarEventsResponseIncluded instantiates a new CalendarEventsResponseIncluded object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCalendarEventsResponseIncluded() *CalendarEventsResponseIncluded {
	this := CalendarEventsResponseIncluded{}
	return &this
}

// NewCalendarEventsResponseIncludedWithDefaults instantiates a new CalendarEventsResponseIncluded object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCalendarEventsResponseIncludedWithDefaults() *CalendarEventsResponseIncluded {
	this := CalendarEventsResponseIncluded{}
	return &this
}

// GetProjects returns the Projects field value if set, zero value otherwise.
func (o *CalendarEventsResponseIncluded) GetProjects() map[string]ViewProject {
	if o == nil || o.Projects == nil {
		var ret map[string]ViewProject
		return ret
	}
	return *o.Projects
}

// GetProjectsOk returns a tuple with the Projects field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CalendarEventsResponseIncluded) GetProjectsOk() (*map[string]ViewProject, bool) {
	if o == nil || o.Projects == nil {
		return nil, false
	}
	return o.Projects, true
}

// HasProjects returns a boolean if a field has been set.
func (o *CalendarEventsResponseIncluded) HasProjects() bool {
	if o != nil && o.Projects != nil {
		return true
	}

	return false
}

// SetProjects gets a reference to the given map[string]ViewProject and assigns it to the Projects field.
func (o *CalendarEventsResponseIncluded) SetProjects(v map[string]ViewProject) {
	o.Projects = &v
}

// GetUsers returns the Users field value if set, zero value otherwise.
func (o *CalendarEventsResponseIncluded) GetUsers() map[string]ViewUser {
	if o == nil || o.Users == nil {
		var ret map[string]ViewUser
		return ret
	}
	return *o.Users
}

// GetUsersOk returns a tuple with the Users field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CalendarEventsResponseIncluded) GetUsersOk() (*map[string]ViewUser, bool) {
	if o == nil || o.Users == nil {
		return nil, false
	}
	return o.Users, true
}

// HasUsers returns a boolean if a field has been set.
func (o *CalendarEventsResponseIncluded) HasUsers() bool {
	if o != nil && o.Users != nil {
		return true
	}

	return false
}

// SetUsers gets a reference to the given map[string]ViewUser and assigns it to the Users field.
func (o *CalendarEventsResponseIncluded) SetUsers(v map[string]ViewUser) {
	o.Users = &v
}

func (o CalendarEventsResponseIncluded) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Projects != nil {
		toSerialize["projects"] = o.Projects
	}
	if o.Users != nil {
		toSerialize["users"] = o.Users
	}
	return json.Marshal(toSerialize)
}

type NullableCalendarEventsResponseIncluded struct {
	value *CalendarEventsResponseIncluded
	isSet bool
}

func (v NullableCalendarEventsResponseIncluded) Get() *CalendarEventsResponseIncluded {
	return v.value
}

func (v *NullableCalendarEventsResponseIncluded) Set(val *CalendarEventsResponseIncluded) {
	v.value = val
	v.isSet = true
}

func (v NullableCalendarEventsResponseIncluded) IsSet() bool {
	return v.isSet
}

func (v *NullableCalendarEventsResponseIncluded) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCalendarEventsResponseIncluded(val *CalendarEventsResponseIncluded) *NullableCalendarEventsResponseIncluded {
	return &NullableCalendarEventsResponseIncluded{value: val, isSet: true}
}

func (v NullableCalendarEventsResponseIncluded) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCalendarEventsResponseIncluded) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


